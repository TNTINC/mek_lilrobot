...
{ (void) curr_speed;
  //RCLCPP_INFO(logger_, "RotateToHeading called with angle: %.2f, speed: %.2f", angle_to_path, curr_speed.angular.z);
  // Rotate in place using max angular velocity / acceleration possible
  linear_vel = 0.0;
  const double sign = angle_to_path > 0.0 ? 1.0 : -1.0;
  
  // Max absolute velocity that still allows us to decelerate in time (sqrt(2*a*theta))
  const double vel_abs_stop_lim = sqrt(2 * max_angular_accel_ * fabs(angle_to_path)) * 0.8;
  double abs_vel = std::min(rotate_to_heading_angular_vel_, vel_abs_stop_lim);

  //RCLCPP_INFO(logger_, "RotateToHeading: vel_abs_stop_lim: %.2f, abs_vel: %.2f", vel_abs_stop_lim, abs_vel);

  // Set the angular velocity
  angular_vel = sign * abs_vel;
}
