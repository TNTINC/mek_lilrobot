...
RangeSensorBroadcaster::RangeSensorBroadcaster()
    : controller_interface::ControllerInterface(){};

CallbackReturn RangeSensorBroadcaster::on_init() {
...
CallbackReturn RangeSensorBroadcaster::on_activate(
    const rclcpp_lifecycle::State &previous_state) {
  this->range_pub_ =
...
InterfaceConfiguration
RangeSensorBroadcaster::state_interface_configuration() const {
...
return_type RangeSensorBroadcaster::update(const rclcpp::Time &time,
                                           const rclcpp::Duration &period) {

  if (state_interfaces_.size() != 1) {
    RCLCPP_ERROR(get_node()->get_logger(),
                 "RangeSensorBroadcaster: Wrong number of state interfaces");
  }
...
PLUGINLIB_EXPORT_CLASS(range_sensor_broadcaster::RangeSensorBroadcaster,
                       controller_interface::ControllerInterface)